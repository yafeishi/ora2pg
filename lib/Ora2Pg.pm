...
sub _init
{
	...
	if ($self->{debug}) {
		$self->logit("Ora2Pg for AntDB version: $VERSION\n");
	}
...
sub data_dump
{
	...
	#$self->logit("antdb debug: rname is $rname,pname is $pname,tname is $tname line 12646\n", 1);
	# if ($self->{file_per_table}) {
			...
			while ( my $rows = $sth->fetchall_arrayref(undef,$data_limit)) {
				...
				if ( ($self->{jobs} > 1) || ($self->{oracle_copies} > 1) ) {
					...
					spawn sub { 
            			...
            			$self->logit("process:$proc,tname:$table,pname:$part_name,current_rows: $current_rows ,is_rename:$is_rename_parallel,file_no:$file_no_proc,split_limit:$self->{split_limit},data_limit:$self->{data_limit}\n");
						$self->_dump_to_pg($proc, $rows, $table, $sql_header,$cmd_head, $cmd_foot, $s_out, $tt, $sprep, $stt, $start_time, $part_name, $total_record, $file_no_proc, $is_rename_parallel,%user_type); # change by antdb
					};
					$self->{child_count}++;
				} else {
					if ($current_rows >= $self->{split_limit} ||  $self->{data_limit} > @$rows) {
				  	...
				  	$self->logit("tname:$table,pname:$part_name,current_rows: $current_rows ,is_rename:$is_rename,file_no:$file_no,split_limit:$self->{split_limit},data_limit:$self->{data_limit}\n");
					$self->_dump_to_pg($proc, $rows, $table, $sql_header,$cmd_head, $cmd_foot, $s_out, $tt, $sprep, $stt, $start_time, $part_name, $total_record, $file_no, $is_rename,%user_type); # change by antdb
					$is_rename = 0;
...
sub _dump_to_pg
{
	...
	else
	{
		...
		if (!$self->{oracle_speed}) {
			$self->data_dump($sql_out, $table, $part_name,$file_no,$is_rename); # change by antdb
		}
	}
